...
class AgentDQN(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        # Other optimizers like Adagrad are also possible
    ...
    def DQN_policy(self, state_representation):
        ...
        with torch.no_grad():
            if not GPU_AVAILABILITY:
                ...
                action = self.dqn.predict(torch.cuda.FloatTensor(state_representation))
        return action
    ...
    def register_experience_replay_tuple(self, s_t, a_t, reward, s_tplus1, episode_over, st_user, from_model=False):
        ...
        # print(self.experience_replay_pool)
        # print(self.experience_replay_pool_from_model)
        ...
        for iter_batch in range(num_batches):
            for iter in range(len(self.running_expereince_pool) / (batch_size)):
                ...
                if not GPU_AVAILABILITY:
                    ...
                    # print("next_state_value_after_unsqueeze: ", next_state_value)
                    ...
                    # print("batch.reward: ", torch.FloatTensor(batch.reward))
                    # print("gamma, next_state_value, torch.FloatTensor(term): ", self.gamma, next_state_value, torch.FloatTensor(term))
                else:
                    state_value = self.dqn(torch.cuda.FloatTensor(batch.state)).gather(1, torch.tensor(batch.action,
                                                                                                       device=DEVICE))
                    next_state_value, _ = self.target_dqn(torch.cuda.FloatTensor(batch.next_state)).max(1)
                    next_state_value = next_state_value.unsqueeze(1)
                    term = np.asarray(batch.term, dtype=np.float32)
                    expected_value = torch.cuda.FloatTensor(batch.reward) + self.gamma * next_state_value * \
                                     (1 - torch.cuda.FloatTensor(term))
